...
#include <ctype.h>
#include <stdlib.h>
// correcteur : pourquoi mettre dans un header quand c est juste utilisé ici ?
#include "shell.h"
#include "mount.h"
#include "sha.h"
#include "error.h"
...
int do_exit(const char (*args)[0]) {
    // correcteur : il faure retourner la valeur de retour de umountv6
    umountv6_fs();
...
int do_mount(const char (*args)[1]) {
    ...
    // correcteur : pourquoi allouer dynamiquement alors qu'une allocation statique suffit ?? + Pas de check sur u == NULL
